...
async def _fetch_overpass(query: str, cache_key: str) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=OVERPASS_TIMEOUT_SECONDS) as client:
        response = await client.post(OVERPASS_BASE, data={"data": query})
...
async def _build_top_pois(
    *,
    ...
    if not requested:
        return []
    try:
        query = _osm_query_for_categories(lat, lng, radio_m, requested)
        payload = await _fetch_overpass(
            query=query,
            cache_key=f"osm-poi:{lat:.4f}:{lng:.4f}:{radio_m}:{','.join(sorted(requested))}",
        )
        ...
        for element in elements:
            ...
            if distancia_m > radio_m:
                continue
            nombre = str(tags.get("name") or f"POI {categoria.title()}")
            proximity_score = max(0.0, 1.0 - (distancia_m / max(radio_m, 1)))
            category_score = _POI_PESO_CATEGORIA.get(categoria, 0.4)
...
async def _fetch_overpass_ways(
    ...
    query = f"[out:json][timeout:20];(way(around:{radio_m},{lat},{lng})['highway'];);out geom 180;"
    payload = await _fetch_overpass(
        query=query,
        cache_key=f"osm-roads:{lat:.4f}:{lng:.4f}:{radio_m}",
    )
...
[out:json][timeout:20];
(
  node(around:{radio_m},{lat},{lng})["amenity"="bus_station"];
  node(around:{radio_m},{lat},{lng})["public_transport"];
  node(around:{radio_m},{lat},{lng})["aeroway"="aerodrome"];
);
out body;
"""
    payload = await _fetch_overpass(
        query=query,
        cache_key=f"osm-nodes:{lat:.4f}:{lng:.4f}:{radio_m}",
    )
    features: list[MapaCapaFeatureResponse] = []
    elements = payload.get("elements")
    if not isinstance(elements, list):
    ...
    for element in elements:
        ...
        if lat_v is None or lng_v is None:
            continue
        tags = element.get("tags") if isinstance(element.get("tags"), dict) else {}
        name = str(tags.get("name", "Nodo de transporte"))
        kind = str(tags.get("amenity") or tags.get("public_transport") or tags.get("aeroway") or "")
        features.append(
            _build_capa_feature(
...
async def _build_capa_datos(
    *,
    ...
    capa_meta = next((c for c in CAPAS_BASE if c.id == capa_id), None)
    nombre = capa_meta.nombre if capa_meta else capa_id
    ...
    try:
        if capa_id == "red_vial":
            ...
            source_type = "official"
        elif capa_id == "nodos_transporte":
            features = await _fetch_overpass_transport_nodes(lat, lng, radio_m)
            source_name = "OpenStreetMap Overpass (nodos transporte)"
            source_type = "official"
        elif capa_id == "ageb_urbana":
            geo = await _fetch_wscatgeo_geojson(f"agebu/{cve_ent}/{cve_mun}")
            raw_features = _as_feature_list(geo)
            features = _extract_polygon_features_in_bbox(raw_features, bbox)
            source_name = "INEGI wscatgeo agebu"
            source_type = "official"
        elif capa_id == "ageb_rural":
            geo = await _fetch_wscatgeo_geojson(f"agebr/{cve_ent}/{cve_mun}")
            raw_features = _as_feature_list(geo)
            features = _extract_polygon_features_in_bbox(raw_features, bbox)
            source_name = "INEGI wscatgeo agebr"
            source_type = "official"
        elif capa_id in {"poi_hoteles", "poi_comercio"}:
            categorias = ["hotelero"] if capa_id == "poi_hoteles" else ["comercial"]
            top = await _build_top_pois(
                lat=lat,
                lng=lng,
                radio_m=radio_m,
                source_mode=source_mode,
                capas_activas=[capa_id],
                limit=50,
...
async def post_mapa_query(
    ...
    capas_activas = body.capas if body.capas else [c.id for c in CAPAS_BASE[:4]]
...
async def get_mapa_capas_datos(
    ...
    if not capas_activas:
        capas_activas = [c.id for c in CAPAS_BASE[:4]]
