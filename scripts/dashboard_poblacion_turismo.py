...
# 1. Inject the option
option_target = '<option value="crecimiento-historico">Crecimiento poblacional anual</option>'
option_new = option_target + '\\n                            <option value="poblacion-ocupada-turismo">Población Ocupada en Restaurantes y Hoteles</option>'
text = text.replace(option_target, option_new)


# 2. Inject the IF block in loadIndicadorCiudad
fetch_target = '''        } else {
            msg.textContent = 'Seleccione un indicador.';
        }
    }

    function renderDistribucionPoblacionCiudad'''

fetch_new = '''        } else if (ind === 'poblacion-ocupada-turismo') {
