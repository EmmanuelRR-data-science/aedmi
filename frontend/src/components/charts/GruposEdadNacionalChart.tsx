...
const GRUPOS = ['0-14 años', '15-64 años', '65+ años'] as const
type Grupo = typeof GRUPOS[number]

export default function GruposEdadNacionalChart() {
  ...
  const indicador = indicadores?.find((i) => i.clave === 'inegi.grupos_edad_nacional')
  ...
  if (isLoading) {
    return (
      <div style={{ background: '#1a1d27', border: '1px solid #2d3148', borderRadius: '10px',
        ...
        Cargando datos de grupos de edad...
      </div>
    )
  }
  ...
  if (!datos.length) {
    return (
      <div style={{ background: '#1a1d27', border: '1px solid #2d3148', borderRadius: '10px',
        ...
        Sin datos disponibles para Distribución por Grupos de Edad.
      </div>
    )
  }

  // Agrupar por año: { anio: { '0-14 años': N, '15-64 años': N, '65+ años': N } }
  const porAnio: Record<string, Record<Grupo, number>> = {}
  for (const d of datos) {
    const anio = String(d.periodo)
    const grupo = d.entidad_clave as Grupo
    if (!porAnio[anio]) porAnio[anio] = { '0-14 años': 0, '15-64 años': 0, '65+ años': 0 }
    if (GRUPOS.includes(grupo)) porAnio[anio][grupo] = d.valor
  }

  const chartData = Object.entries(porAnio)
    .sort(([a], [b]) => Number(a) - Number(b))
    .map(([anio, grupos]) => ({ anio, ...grupos }))

  const maxValor = Math.max(...chartData.flatMap((d) => GRUPOS.map((g) => d[g] ?? 0)))
  ...
  onDownloadPng={() => downloadChartAsPng(chartRef, 'distribucion-poblacion-grupos-edad')}
/>
      ...
      <div ref={chartRef}>
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={chartData} margin={{ top: 24, right: 16, left: 0, bottom: 4 }}>
            ...
            {GRUPOS.map((grupo, i) => (
              <Bar key={grupo} dataKey={grupo} fill={palette[i % palette.length] ?? '#0576F3'}
                radius={[3, 3, 0, 0]} name={grupo}>
                <LabelList dataKey={grupo} position="top"
      ...
      <p style={{ fontSize: '11px', color: '#4a5568', fontFamily, margin: '-8px 0 0 0', textAlign: 'left' }}>
        Fuente: INEGI — Censos y Conteos de Población y Vivienda (2000, 2005, 2010, 2020)
      </p>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', fontFamily, color: '#94a3b8' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #2d3148' }}>
              ...
              {GRUPOS.map((g) => (
                <th key={g} style={{ textAlign: 'right', padding: '6px 8px', color: '#64748b' }}>{g}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {chartData.map((d) => (
              <tr key={d.anio} style={{ borderBottom: '1px solid #1e2235' }}>
                ...
                {GRUPOS.map((g) => (
                  <td key={g} style={{ padding: '5px 8px', textAlign: 'right' }}>
                    {new Intl.NumberFormat('es-MX').format(d[g] ?? 0)}
                  </td>
                ))}
              </tr>
            ))}
